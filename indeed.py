...
class IndeedJobList(JobList):
    ...
    Background:
    =====================
    There used to be a link from any search that would take you to the rss feed
    directly in the browser and you could just copy the link; in any case,
    ...
    * offset: The rss returns up to 20 results, you can page through them
    ...
    :param max_results: int A maximum number of results. The results
    may be less than this, but the function will stop querying if this number
    is reached.

    :param oldest: timedelta Anything older than today - oldest will be ignored.

    :returns: A generator which when called will yield a dict of the following
    format:
     {
    ...
    def collect_results(self, keywords, location, radius, filter_location=(),
        ...
        while found < max_results:
            # Get a page of feed results (sorted by date), and process it
            # until either a date older than *oldest* appears or all the
